    ...
    def evaluate(self):
        ...
        for sample in self.dataloader:
            ...
            if sample["path"][0][0].split(".")[-1] in ["jpg", "JPG"]:
                ...
                print("metadata tagging only supported for jpgs", flush=True)
            ...
            # Print prediction immediately with flush to see in real-time
            filename = os.path.basename(sample["path"][0][0])
            predicted_focal = output.cpu().item()
            print(f"{filename} Predicted {predicted_focal:5.1f}mm", flush=True)
